...
public partial class LegacyMainMenu : Control
{
	...
	public override void _Ready()
	{
		...
		{
			Initialized = true;
			SoundManager.Instance.JukeboxPlayed += (Map map) =>
			{
				PassedNotes = 0;
				CurrentMap = map;
			};

			Viewport viewport = SceneManager.Instance.GetViewport();
		...
		{
			Jukebox.GetNode<Label>("Title").Text = LegacyRunner.CurrentAttempt.Map.PrettyTitle;
			SoundManager.JukeboxPaused = false;
			CurrentMap = LegacyRunner.CurrentAttempt.Map;
	...
	public static void UpdateMapList()
	{
		...
		foreach (string mapFile in Directory.GetFiles($"{Constants.USER_FOLDER}/maps"))
		{
			try
			{
				...
				holder.GetNode<RichTextLabel>("Extra").Text = $"[color={Constants.DIFFICULTY_COLORS[difficulty].ToHtml(false)}]{difficultyName}[color=808080] - {mappers}".ReplaceLineEndings("");
