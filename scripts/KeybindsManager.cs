            ...
            if (eventMouseButton.CtrlPressed && (eventMouseButton.ButtonIndex == MouseButton.WheelUp || eventMouseButton.ButtonIndex == MouseButton.WheelDown))
            {
                switch (eventMouseButton.ButtonIndex)
                {
                    case MouseButton.WheelUp:
                        SettingsManager.Instance.Settings.VolumeMaster.Value = Mathf.Min(100, settings.VolumeMaster + 5f);
                        break;
                    case MouseButton.WheelDown:
                        SettingsManager.Instance.Settings.VolumeMaster.Value = Mathf.Max(0, settings.VolumeMaster - 5f);
                        break;
                }
